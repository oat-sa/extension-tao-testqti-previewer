define("tpl!taoQtiTestPreviewer/previewer/runner",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},'<div class="previewer-component"></div>'})}),define("taoQtiTestPreviewer/previewer/runner",["jquery","lodash","context","core/promise","taoTests/runner/runnerComponent","tpl!taoQtiTestPreviewer/previewer/runner"],function($,_,context,Promise,runnerComponentFactory,runnerTpl){"use strict";var defaults={};return function(config,container,template){return(config=_.defaults(config||{},defaults)).providers&&(
config.providers=_.filter(config.providers,{category:"previewer"})),runnerComponentFactory(container||$(document.body),config,template||runnerTpl).on("render",function(){this.setState("fullpage",config.fullPage),this.setState("readonly",config.readOnly)}).on("ready",function(runner){var self=this;runner.on("destroy",function(){self.destroy()})})}}),define("css!taoQtiTestPreviewer/previewer/provider/item/css/item",[],function(){}),define("taoQtiTestPreviewer/previewer/adapter/item/qtiItem",["lodash","core/logger","taoQtiTestPreviewer/previewer/runner","css!taoQtiTestPreviewer/previewer/provider/item/css/item"],function(_,loggerFactory,previewerFactory){
"use strict";var logger=loggerFactory("taoQtiTest/previewer"),defaultProviders=[{id:"qtiItemPreviewer",module:"taoQtiTestPreviewer/previewer/provider/item/item",bundle:"taoQtiTestPreviewer/loader/qtiPreviewer.min",category:"previewer"}],defaultPlugins=[{module:"taoQtiTestPreviewer/previewer/plugins/controls/close",bundle:"taoQtiTestPreviewer/loader/qtiPreviewer.min",category:"controls"},{module:"taoQtiTestPreviewer/previewer/plugins/navigation/submit/submit",bundle:"taoQtiTestPreviewer/loader/qtiPreviewer.min",category:"navigation"},{module:"taoQtiTest/runner/plugins/tools/itemThemeSwitcher/itemThemeSwitcher",bundle:"taoQtiTest/loader/testPlugins.min",
category:"tools"}];return{name:"qtiItem",init:function(uri,state,config){return(config=_.defaults(config||{},{provider:"qtiItemPreviewer",serviceCallId:"previewer"})).providers=defaultProviders.concat(config.providers||[]),config.plugins=defaultPlugins.concat(config.plugins||[]),previewerFactory(config).on("error",function(err){logger.error(err)}).on("ready",function(runner){runner.on("renderitem",function(){state&&runner.itemRunner.setState(state)}).loadItem(uri)})}}}),define("taoQtiTestPreviewer/previewer/config/item",["lodash","util/url","util/config"],function(_,urlUtil,configHelper){"use strict";var _defaults={bootstrap:{serviceController:"Previewer",
serviceExtension:"taoQtiTestPreviewer"}},_entries={serviceCallId:!0,bootstrap:!1,timeout:!1};return function(config){var storage=configHelper.from(config,_entries,_defaults);return storage.timeout?storage.timeout*=1e3:storage.timeout=void 0,{getParameters:function(itemIdentifier){var type=typeof itemIdentifier,parameters={serviceCallId:this.getServiceCallId()};if("string"===type)parameters.itemUri=itemIdentifier;else if("object"===type&&_.isPlainObject(itemIdentifier))_.merge(parameters,itemIdentifier);else if("undefined"!==type)throw new TypeError("Wrong parameter type provided for itemIdentifier: "+type+". Only string or plain object are allowed!")
;return parameters},getServiceCallId:function(){return storage.serviceCallId},getServiceController:function(){return storage.bootstrap.serviceController||_defaults.bootstrap.serviceController},getServiceExtension:function(){return storage.bootstrap.serviceExtension||_defaults.bootstrap.serviceExtension},getTestActionUrl:function(action){return urlUtil.route(action,this.getServiceController(),this.getServiceExtension(),this.getParameters())},getItemActionUrl:function(itemIdentifier,action){return urlUtil.route(action,this.getServiceController(),this.getServiceExtension(),this.getParameters(itemIdentifier))},getTimeout:function(){return storage.timeout}}}}),
define("taoQtiTestPreviewer/previewer/plugins/controls/close",["jquery","lodash","i18n","ui/hider","taoTests/runner/plugin","tpl!taoQtiTest/runner/plugins/templates/button"],function($,_,__,hider,pluginFactory,buttonTpl){"use strict";return pluginFactory({name:"close",init:function(){var self=this,testRunner=this.getTestRunner();this.$element=$(buttonTpl({control:"close",title:__("Close the previewer"),icon:"close",text:__("Close"),className:"context-action"})),this.$element.on("click",function(e){e.preventDefault(),!1!==self.getState("enabled")&&(self.disable(),testRunner.trigger("finish"))}),this.disable(),testRunner.on("enablenav",function(){self.enable()
}).on("disablenav",function(){self.disable()})},render:function(){this.getAreaBroker().getArea("context").append(this.$element)},destroy:function(){this.$element.remove()},enable:function(){this.$element.removeProp("disabled").removeClass("disabled")},disable:function(){this.$element.prop("disabled",!0).addClass("disabled")},show:function(){hider.show(this.$element)},hide:function(){hider.hide(this.$element)}})}),define("tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,
Handlebars.helpers),data=data||{},'<div class="preview-console hidden">\n    <ul class="preview-console-body"></ul>\n</div>'})}),define("tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console-line",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{};var helper,buffer="",escapeExpression=this.escapeExpression;return buffer+='<li><pre><span class="log-time">',buffer+=escapeExpression((helper=helpers.time)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.time)?helper.call(
depth0,{hash:{},data:data}):helper)+'</span><span class="log-type">',buffer+=escapeExpression((helper=helpers.type)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.type)?helper.call(depth0,{hash:{},data:data}):helper)+'</span><span class="log-message">',buffer+=escapeExpression((helper=helpers.message)?helper.call(depth0,{hash:{},data:data}):"function"==typeof(helper=depth0&&depth0.message)?helper.call(depth0,{hash:{},data:data}):helper)+"</span></pre></li>"})}),define("tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console-closer",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,
helpers,partials,data){this.compilerInfo=[4,">= 1.0.0"],data=data||{};var helper,options,buffer="",helperMissing=(helpers=this.merge(helpers,Handlebars.helpers)).helperMissing;return buffer+='<span class="icon-close preview-console-closer hidden" title="'+(0,this.escapeExpression)((options={hash:{},data:data},(helper=helpers.__||depth0&&depth0.__)?helper.call(depth0,"Close console",options):helperMissing.call(depth0,"__","Close console",options)))+'"></span>'})}),define("taoQtiTestPreviewer/previewer/plugins/navigation/submit/submit",["jquery","lodash","i18n","moment","ui/hider","ui/autoscroll","util/strPad","taoTests/runner/plugin",
"taoQtiItem/qtiCommonRenderer/helpers/PciResponse","tpl!taoQtiTest/runner/plugins/templates/button","tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console","tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console-line","tpl!taoQtiTestPreviewer/previewer/plugins/navigation/submit/preview-console-closer"],function($,_,__,moment,hider,autoscroll,strPad,pluginFactory,pciResponse,buttonTpl,consoleTpl,consoleLineTpl,consoleCloserTpl){"use strict";return pluginFactory({name:"submit",init:function(){var self=this,testRunner=this.getTestRunner();function isPluginAllowed(){return!testRunner.getConfig().readOnly}
function showConsole(){hider.show(self.controls.$console),hider.show(self.controls.$consoleBody),hider.show(self.controls.$consoleCloser),autoscroll(self.controls.$consoleBody.children().last(),self.controls.$consoleBody)}function showResponses(type,responses){_.forEach(responses,function(response,identifier){!function(type,message){var data={time:strPad(moment().format("HH:mm:ss"),12," "),type:strPad(type||"",18," "),message:strPad(message||"",18," ")};self.controls.$consoleBody.append($(consoleLineTpl(data)))}(type,strPad(identifier+": ",15," ")+_.escape(pciResponse.prettyPrint(response)))})}this.controls={$button:$(buttonTpl({control:"submit",title:__(
"Submit and show the result"),icon:"forward",text:__("Submit")})),$console:$(consoleTpl()),$consoleCloser:$(consoleCloserTpl())},this.controls.$consoleBody=this.controls.$console.find(".preview-console-body"),this.controls.$button.on("click",function(e){e.preventDefault(),!1!==self.getState("enabled")&&(self.disable(),testRunner.trigger("submititem"))}),this.controls.$consoleCloser.on("click",function(e){e.preventDefault(),hider.hide(self.controls.$console),hider.hide(self.controls.$consoleCloser)}),isPluginAllowed()||this.hide(),this.disable(),testRunner.on("render",function(){isPluginAllowed()?self.show():self.hide()}).on("submitresponse",function(
responses){showResponses(__("Submitted data"),responses),showConsole()}).on("scoreitem",function(responses){responses.itemSession&&(showResponses(__("Output data"),responses.itemSession),showConsole())}).on("enablenav",function(){self.enable()}).on("disablenav",function(){self.disable()})},render:function(){var $container=this.getAreaBroker().getContainer(),$navigation=this.getAreaBroker().getNavigationArea();$navigation.append(this.controls.$button),$navigation.append(this.controls.$consoleCloser),$container.append(this.controls.$console)},destroy:function(){_.forEach(this.controls,function($el){$el.remove()}),this.controls=null},enable:function(){
this.controls.$button.removeProp("disabled").removeClass("disabled")},disable:function(){this.controls.$button.prop("disabled",!0).addClass("disabled")},show:function(){hider.show(this.controls.$button)},hide:function(){_.forEach(this.controls,hider.hide)}})}),define("taoQtiTestPreviewer/previewer/proxy/item",["jquery","lodash","i18n","core/promise","core/promiseQueue","core/communicator","taoQtiTestPreviewer/previewer/config/item","util/httpErrorParser"],function($,_,__,Promise,promiseQueue,communicatorFactory,configFactory,httpErrorParser){"use strict";return{install:function(){var self=this;this.queue=promiseQueue(),this.prepareParams=function(
...
"request"),resolve(data);reject(httpErrorParser.parse(jqXHR,textStatus,errorThrown))})})};return!0===noToken?runRequest():this.queue.serie(runRequest)}},init:function(config,params){return this.configStorage=configFactory(config||{}),this.request(this.configStorage.getTestActionUrl("init"),params)},destroy:function(){return this.configStorage=null,this.queue=null,Promise.resolve()},callTestAction:function(action,params){return this.request(this.configStorage.getTestActionUrl(action),params)},callItemAction:function(itemIdentifier,action,params){return this.request(this.configStorage.getItemActionUrl(itemIdentifier,action),params)},getItem:function(
itemIdentifier,params){return this.request(this.configStorage.getItemActionUrl(itemIdentifier,"getItem"),params)},submitItem:function(itemIdentifier,state,response,params){var body=_.merge({itemState:state,itemResponse:response},params||{});return this.request(this.configStorage.getItemActionUrl(itemIdentifier,"submitItem"),body)}}}),define("tpl!taoQtiTestPreviewer/previewer/provider/item/tpl/item",["handlebars"],function(hb){return hb.template(function(Handlebars,depth0,helpers,partials,data){return this.compilerInfo=[4,">= 1.0.0"],helpers=this.merge(helpers,Handlebars.helpers),data=data||{},
...
;return proxyFactory.registerProvider("qtiItemPreviewerProxy",proxyProvider),{name:"qtiItemPreviewer",loadAreaBroker:function(){var $layout=$(layoutTpl());return areaBrokerFactory($layout,{content:$("#qti-content",$layout),toolbox:$(".bottom-action-bar .tools-box",$layout),navigation:$(".bottom-action-bar .navi-box-list",$layout),control:$(".top-action-bar .control-box",$layout),actionsBar:$(".bottom-action-bar .control-box",$layout),panel:$(".test-sidebar-left",$layout),header:$(".top-action-bar .tools-box",$layout),context:$(".top-action-bar .navi-box-list",$layout)})},loadProxy:function(){var config=this.getConfig(),proxyConfig=_.pick(config,[
"serviceCallId","bootstrap","timeout"]);return proxyFactory(config.proxyProvider||"qtiItemPreviewerProxy",proxyConfig)},loadTestStore:function(){var identifier=this.getConfig().serviceCallId||"test-"+Date.now();return testStoreFactory(identifier)},init:function(){var self=this,dataHolder=this.getDataHolder(),areaBroker=this.getAreaBroker();return areaBroker.setComponent("toolbox",toolboxFactory()),areaBroker.getToolbox().init(),this.on("submititem",function(){var itemRunner=this.itemRunner,itemState=itemRunner.getState(),itemResponses=itemRunner.getResponses();return this.trigger("disabletools disablenav"),this.trigger("submitresponse",itemResponses,
itemState),this.getProxy().submitItem(dataHolder.get("itemIdentifier"),itemState,itemResponses).then(function(response){self.trigger("scoreitem",response),self.trigger("enabletools enablenav resumeitem")}).catch(function(err){self.trigger("enabletools enablenav"),200===err.code&&self.trigger("alert.submitError",err.message||__("An error occurred during results submission. Please retry."),function(){self.trigger("resumeitem")})})}).on("ready",function(){var itemIdentifier=dataHolder.get("itemIdentifier"),itemData=dataHolder.get("itemData");itemIdentifier&&(itemData?self.renderItem(itemIdentifier,itemData):self.loadItem(itemIdentifier))}).on("loaditem",
function(itemRef,itemData){dataHolder.set("itemIdentifier",itemRef),dataHolder.set("itemData",itemData)}).on("renderitem",function(){this.trigger("enabletools enablenav")}).on("resumeitem",function(){this.trigger("enableitem enablenav")}).on("disableitem",function(){this.trigger("disabletools")}).on("enableitem",function(){this.trigger("enabletools")}).on("error",function(){this.trigger("disabletools enablenav")}).on("finish leave",function(){this.trigger("disablenav disabletools"),this.flush()}).on("flush",function(){this.destroy()}),this.getProxy().init().then(function(data){dataHolder.set("itemIdentifier",data.itemIdentifier),dataHolder.set("itemData",
data.itemData)})},render:function(){var config=this.getConfig(),areaBroker=this.getAreaBroker();config.renderTo.append(areaBroker.getContainer()),areaBroker.getToolbox().render(areaBroker.getToolboxArea())},loadItem:function(itemIdentifier){return this.getProxy().getItem(itemIdentifier)},renderItem:function(itemIdentifier,itemData){var self=this,changeState=function(){self.setItemState(itemIdentifier,"changed",!0)};return new Promise(function(resolve,reject){assetManager.setData("baseUrl",itemData.baseUrl),itemData.content=itemData.content||{},self.itemRunner=qtiItemRunner(itemData.content.type,itemData.content.data,{assetManager:assetManager}).on("error",
function(err){self.trigger("enablenav"),reject(err)}).on("init",function(){var itemContainer=self.getAreaBroker().getContentArea(),options=_.pick(itemData,["state","portableElements"]);this.render(itemContainer,options)}).on("render",function(){this.on("responsechange",changeState),this.on("statechange",changeState),resolve()}).init()})},unloadItem:function(){var self=this;return self.trigger("beforeunloaditem disablenav disabletools"),new Promise(function(resolve){self.itemRunner?self.itemRunner.on("clear",resolve).clear():resolve()})},destroy:function(){var areaBroker=this.getAreaBroker();this.itemRunner&&this.itemRunner.clear(),this.itemRunner=null,
areaBroker&&areaBroker.getToolbox().destroy()}}}),function(c){var d=document,a="appendChild",i="styleSheet",s=d.createElement("style");s.type="text/css",d.getElementsByTagName("head")[0][a](s),s[i]?s[i].cssText=c:s[a](d.createTextNode(c))}(
...
),define("taoQtiTestPreviewerloaderqtiPreviewer.bundle",function(){}),define("taoQtiTestPreviewerloaderqtiPreviewer.min",["taoItems/loader/taoItemsRunner.min","taoTests/loader/taoTestsRunner.min","taoQtiItem/loader/taoQtiItemRunner.min","taoQtiTest/loader/taoQtiTestRunner.min","taoQtiTest/loader/testPlugins.min"],function(){});
